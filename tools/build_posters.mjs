 ...
 *   node tools/build_posters.mjs --derivatives-only  # render yok, türevleri yenile
 *
...
/** Kart ~355 CSS px genişlikte gösteriliyor; 800 px 2x ekranları karşılar.
 *  Ana dosya (1600) görüntüleyici posteri ve büyük ekranlar için kalır. */
const DERIVATIVE_WIDTH = 800;
...
/** Ana posterden küçük ekran türevleri (yeniden render gerekmez). */
function buildDerivatives(masterWebp, id) {
  const height = Math.round(DERIVATIVE_WIDTH * HERO.height / HERO.width);
  for (const [extension, quality] of [['webp', '78'], ['avif', '50']]) {
    const output = path.join(POSTER_DIR, `${id}@${DERIVATIVE_WIDTH}.${extension}`);
    magick([
      masterWebp, '-alpha', 'set',
      '-resize', `${DERIVATIVE_WIDTH}x${height}`,
      '-quality', quality,
      output,
    ]);
  }
...
async function main() {
  ...
  // Yalnızca türevleri tazele: ana posterler zaten üretilmişse render
  // gerektirmez (kalite/boyut denemeleri için hızlı yol).
  if (options.derivativesOnly !== undefined) {
    const lqipOnly = {};
    for (const model of models) {
      const id = String(model.id);
      const master = path.join(POSTER_DIR, `${id}.webp`);
      try {
        buildDerivatives(master, id);
        lqipOnly[id] = buildLqip(master);
        console.log(`  ✓ ${id}: ${DERIVATIVE_WIDTH} px türevler yenilendi`);
      } catch (error) {
        console.error(`  ! ${id}: ana poster yok veya okunamadı (${error.message.slice(0, 60)})`);
      }
    }
    const rules = Object.entries(lqipOnly)
