...
SCALES = np.arange(0.10, 0.46, 0.02)
...
def locate(model_id: str, plan: np.ndarray, plan_alpha: np.ndarray) -> dict | None:
...
def main() -> int:
  ...
  print(f"Plan: {plan_size[0]}x{plan_size[1]} → çalışma {plan.shape[2]}x{plan.shape[1]}, "
        f"{len(SCALES)} ölçek × {len(list(ANGLES))} açı\n")
  ...
  for model in models:
    ...
    best = locate(model_id, plan, plan_alpha)
    if not best:
      continue
